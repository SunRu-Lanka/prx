...
func DwTimeProcess(r *http.Request) ([]DwellTimeSt, error) {
	...
	dwTime := DwellTimeSt{}
	rows,err := config.DB.Query("SELECT date(capturedtime) as CpaturedDate,hour(timediff(max(capturedtime), min(capturedtime))) as hours , minute(timediff(max(capturedtime), min(capturedtime)))/60 as minutes,(hour(timediff(max(capturedtime), min(capturedtime))) + minute(timediff(max(capturedtime), min(capturedtime)))/60) as CapturedTimeDw	FROM proximity.`6c-3b-6b-68-e4-f3`where capturedtime between '2020-01-10' and '2020-02-28' group by date(capturedtime)")
	if err != nil{
